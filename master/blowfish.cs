...
//Modified by Pavel Sudakov. Thanks to aluigi for C source code
...
namespace BlowFishCS
{
    class BlowFish
    {
        ...
        private void SetupKey(byte[] cipherKey)
        {
            ...
            for (int i = 0; i < 18; i++)
            {
                ulong d = (ulong)(((key[j % cipherKey.Length] * 256 + key[(j + 1) % cipherKey.Length]) * 256 + key[(j + 2) % cipherKey.Length]) * 256 + key[(j + 3) % cipherKey.Length]);
                bf_P[i] ^= d;
        ...
        /// Sets up the S-blocks and the key
        /// </summary>
        /// <param name="cipherKey7">Block cipher key (1-448 bits)</param>
        private void SetupKey7(byte[] cipherKey7)
        {
            bf_P = SetupP();
            //set up the S blocks
            bf_s0 = SetupS0();
            ...
            key = new byte[cipherKey7.Length]; // 448 bits
            if (cipherKey7.Length > 56)
            {
                throw new Exception("Key too long. 56 bytes required.");
            }

            Buffer.BlockCopy(cipherKey7, 0, key, 0, cipherKey7.Length);
            int j = 0;
            for (int i = 0; i < 18; i++)
            {
                ulong d = (ulong)(((key[j % cipherKey7.Length] * 256 + key[(j + 1) % cipherKey7.Length]) * 256 + key[(j + 2) % cipherKey7.Length]) * 256 + key[(j + 3) % cipherKey7.Length]);
                bf_P[i] ^= d;
                j = (j + 4) % cipherKey7.Length;
            }

            bf_s0[118] = bswap(bf_s0[118]);
        ...
        public byte[] Crypt_ECB(byte[] text, int version, bool decrypt)
        {
            ...
            {
                Buffer.BlockCopy(text, offset, block, 0, 8);
                
                //Buffer.BlockCopy(plainText, i, block, 0, 8);
                if (decrypt)
                {
                    if (version >= 7) BlockDecrypt7(ref block);
                    else BlockDecrypt(ref block);
                }
                else
                {
                    if (version >= 7) BlockEncrypt7(ref block);
                    else BlockEncrypt(ref block);
                }
                Buffer.BlockCopy(block, 0, text, offset, 8);
                offset += 8;
        ...
        /// </summary>
        /// <param name="block">The 64 bit block to encrypt</param>
        private void BlockEncrypt7(ref byte[] block)
        {
            SetBlock(block);
            encipher7();
            GetBlock(ref block);
        ...
        /// </summary>
        /// <param name="block">The 64 bit block to decrypt</param>
        private void BlockDecrypt7(ref byte[] block)
        {
            SetBlock(block);
            decipher7();
            GetBlock(ref block);
        ...
        /// <summary>
        /// Runs the blowfish algorithm (standard 16 rounds) - mod for versions > 7
        /// </summary>
        private void encipher7()
        {
            xl_par ^= bf_P[0];
            for (uint i = 0; i < ROUNDS; i += 2)
            {
                xr_par = round7(xr_par, xl_par, i + 1);
                xl_par = round7(xl_par, xr_par, i + 2);
            }
            xr_par = xr_par ^ bf_P[17];
        ...
        /// <summary>
        /// Runs the blowfish algorithm in reverse (standard 16 rounds) - mod for version 7
        /// </summary>
        private void decipher7()
        {
            xl_par ^= bf_P[17];
            for (uint i = 16; i > 0; i -= 2)
            {
                xr_par = round7(xr_par, xl_par, i);
                xl_par = round7(xl_par, xr_par, i - 1);
            }
            xr_par = xr_par ^ bf_P[0];
        ...
        /// <returns></returns>
        private ulong round(ulong a, ulong b, uint n)
        {
            ulong x1 = (bf_s0[wordByte0(b)] + bf_s1[wordByte1(b)]) ^ bf_s2[wordByte2(b)];
            ulong x2 = x1 + bf_s3[this.wordByte3(b)];
            ulong x3 = x2 ^ bf_P[n];
            return x3 ^ a;
        }
        ...
        /// <param name="n">See spec</param>
        /// <returns></returns>
        private ulong round7(ulong a, ulong b, uint n)
        {
